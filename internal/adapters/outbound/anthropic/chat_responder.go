...
// ChatResponder answers scoped follow-up questions via Anthropic.
type ChatResponder struct {
	client *Client
	loader prompts.Loader
}

// NewChatResponder builds an Anthropic-backed chat responder.
...
// Answer issues a Messages API call constrained to the active analysis context.
func (responder *ChatResponder) Answer(ctx context.Context, analysisCtx domain.AnalysisContext, question domain.ChatQuestion) (domain.ChatAnswer, error) {
	...
	if err != nil {
		return domain.ChatAnswer{}, fmt.Errorf("anthropic chat call: %w", err)
	}
...
func decodeChatAnswerPayload(content string, analysisID string) (domain.ChatAnswer, error) {
	...
	if trimmed == "" {
		return domain.ChatAnswer{}, fmt.Errorf("anthropic returned empty chat answer")
	}

	var payload chatAnswerPayload
	if err := json.Unmarshal([]byte(trimmed), &payload); err != nil {
		return domain.ChatAnswer{}, fmt.Errorf("decode anthropic chat json: %w", err)
	}
	if payload.Answer == "" {
		return domain.ChatAnswer{}, fmt.Errorf("anthropic chat answer is empty")
	}
