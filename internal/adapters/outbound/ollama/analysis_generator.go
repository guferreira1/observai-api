...
// AnalysisGenerator turns normalized evidence into an analysis result via Ollama.
type AnalysisGenerator struct {
	client *Client
	loader prompts.Loader
}

// NewAnalysisGenerator builds an Ollama-backed analysis generator.
...
// Generate renders a system prompt + user payload, sends it to Ollama with
// JSON-output mode and decodes the response into a domain.AnalysisResult.
func (generator *AnalysisGenerator) Generate(ctx context.Context, request domain.AnalysisRequest, evidence []domain.Evidence) (domain.AnalysisResult, error) {
	...
	if err != nil {
		return domain.AnalysisResult{}, fmt.Errorf("ollama analysis call: %w", err)
	}
...
func decodeAnalysisPayload(content string, evidence []domain.Evidence) (domain.AnalysisResult, error) {
	...
	if trimmed == "" {
		return domain.AnalysisResult{}, fmt.Errorf("ollama returned empty analysis content")
	}

	var payload analysisPayload
	if err := json.Unmarshal([]byte(trimmed), &payload); err != nil {
		return domain.AnalysisResult{}, fmt.Errorf("decode ollama analysis json: %w", err)
	}
